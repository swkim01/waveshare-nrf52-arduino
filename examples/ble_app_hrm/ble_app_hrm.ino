...
const int ECG_PIN = A1;       // the number of the ECG pin (analog)

// pins for leads off detection
const int LEADS_OFF_PLUS_PIN  = 42;      // the number of the LO+ pin (digital)
const int LEADS_OFF_MINUS_PIN = 43;      // the number of the LO- pin (digital) 
...
void setupHRM(void)
{
  ...
  bslc.setFixedLen(1);
  bslc.begin();
  bslc.write8(1);    // Set the characteristic to 'Chest' (1)
}
...
void loop()
{
  ...
  if ( Bluefruit.connected() ) {
    bpmvalue = calcBPM(ECG_PIN, LEADS_OFF_PLUS_PIN, LEADS_OFF_MINUS_PIN);
    if (bpmvalue != 0) {
      uint8_t hrmdata[2] = { 0b00000110, bpmvalue }; // Sensor connected, BPM value
    
      // Note: We use .notify instead of .write!
      // If it is connected but CCCD is not enabled
      // The characteristic's value is still updated although notification is not sent
      if ( hrmc.notify(hrmdata, sizeof(hrmdata)) ){
        Serial.print("Heart Rate Measurement updated to: "); Serial.println(bpmvalue); 
      }else{
